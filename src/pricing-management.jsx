...
// function RadioCard({ value, current, onChange, icon: Icon, label, description, color = 'indigo' }) {
//   const active = current === value;

//   const activeClass = {
//     indigo: 'border-indigo-500/50 bg-indigo-500/10 text-indigo-700 dark:text-indigo-300',
//     sky:    'border-sky-500/50 bg-sky-500/10 text-sky-700 dark:text-sky-300',
//     amber:  'border-amber-500/50 bg-amber-500/10 text-amber-800 dark:text-amber-300',
//   }[color];

//   const inactiveClass = 'border-slate-300 text-slate-700 hover:bg-slate-50 dark:border-white/10 dark:text-slate-300 dark:hover:bg-white/5';

//   return (
//     <button
//       type="button"
//       onClick={() => onChange(value)}
//       className={`flex-1 flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl border transition-colors text-center cursor-pointer ${active ? activeClass : inactiveClass}`}
//     >
//       <div className={`w-8 h-8 rounded-full flex items-center justify-center ${active ? 'bg-current/10' : 'bg-slate-100 dark:bg-white/5'}`}>
...
//   activeClass = 'bg-indigo-600 text-white border-indigo-500/40',
//   inactiveClass = 'bg-transparent border-slate-300 text-slate-700 hover:bg-slate-50 dark:border-white/10 dark:text-slate-300 dark:hover:bg-white/5',
// }) {
//   const active = current === value;
//   return (
//     <button
//       type="button"
//       onClick={() => onChange(value)}
//       className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${active ? activeClass : inactiveClass}`}
//     >
//       {label}
//     </button>
//   );
...
//       type="button"
//       onClick={onClick}
//       className={`px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-colors ${active ? activeClass : inactiveClass}`}
//     >
//       {label}
//     </button>
//   );
...
//   const [collapsedCategories, setCollapsedCategories]     = useState({}); // true => fechado
//   const [collapsedSeasonals, setCollapsedSeasonals]       = useState({}); // true => aberto (mantive sua lógica)
...
//   // Day Use (Categoria) - agora com modo de cobrança
//   const [formDayUseAtivo, setFormDayUseAtivo]               = useState(false);
//   const [formDayUseModo, setFormDayUseModo]                 = useState('padrao'); // 'padrao' | 'ocupacao'
...
//   const [formDayUsePrecosOcupacao, setFormDayUsePrecosOcupacao] = useState({});
//   const [formDayUseHoraAdicionalPorPessoa, setFormDayUseHoraAdicionalPorPessoa] = useState(0);
...
//   // Day Use sazonal - agora com modo de cobrança
//   const [sDayUseAtivo, setSDayUseAtivo]       = useState(false);
//   const [sDayUseModo, setSDayUseModo]         = useState('padrao'); // 'padrao' | 'ocupacao'
//   const [sDayUsePreco, setSDayUsePreco]       = useState(0);
//   const [sDayUsePrecoAd, setSDayUsePrecoAd]   = useState(0);
//   const [sDayUsePrecosOcupacao, setSDayUsePrecosOcupacao] = useState({});
//   const [sDayUseHoraAdicionalPorPessoa, setSDayUseHoraAdicionalPorPessoa] = useState(0);
...
//   // ── Theme (mais empresarial + corrigido) ─────────────────────────────────────
//   const theme = {
//     bg:        isDark ? 'bg-slate-950' : 'bg-slate-50',
//     bgOverlay: isDark ? 'bg-gradient-to-b from-slate-950 via-slate-950 to-slate-900' : 'bg-gradient-to-b from-slate-50 via-slate-50 to-white',
...
//     cardHover:     isDark ? 'hover:bg-white/7' : 'hover:bg-slate-50',
//     tableHeader:   isDark ? 'bg-white/5' : 'bg-slate-100',
//     divider:       isDark ? 'border-white/10' : 'border-slate-200',

//     input:         isDark ? 'bg-slate-900 border-white/10 text-slate-100 placeholder-slate-500'
//                           : 'bg-white border-slate-300 text-slate-900 placeholder-slate-400',

//     button:        isDark ? 'bg-white/5 hover:bg-white/10 border-white/10'
//                           : 'bg-white hover:bg-slate-50 border-slate-300',

//     sectionBg:     isDark ? 'bg-white/3 border-white/10' : 'bg-slate-50 border-slate-200',

//     chip:          isDark ? 'bg-white/5 border-white/10 text-slate-200'
//                           : 'bg-slate-100 border-slate-200 text-slate-700',
//   };

//   const inputCls = `w-full px-3 py-2 ${theme.input} rounded-lg text-sm border focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors`;
...
//   const getMinPreco = (c) =>
//     c.modeloCobranca === 'Por quarto (tarifa fixa)'
//       ? c.precoFixo
//       : Math.min(...Object.values(c.precosOcupacao || { 0: 0 }));

//   const getMaxPreco = (c) =>
//     c.modeloCobranca === 'Por quarto (tarifa fixa)'
//       ? c.precoFixo
//       : Math.max(...Object.values(c.precosOcupacao || { 0: 0 }));

//   const getQuartosVinc = (cat) => baseRooms.filter((r) => cat.vinculoQuartos.includes(r.numero));

//   const normalizeDayUse = (du, maxPessoas) => {
//     const modo = du?.modo || 'padrao';
//     const base = {
//       ativo: !!du?.ativo,
//       modo,
//       precoFixo: Number(du?.precoFixo || 0),
//       precoAdicional: Number(du?.precoAdicional || 0),
//       precosOcupacao: du?.precosOcupacao || {},
//       horaAdicionalPorPessoa: Number(du?.horaAdicionalPorPessoa || 0),
//     };

//     if (modo === 'ocupacao') {
//       const n = {};
//       for (let i = 1; i <= (maxPessoas || 1); i++) n[i] = Number(base.precosOcupacao?.[i] || 0);
//       base.precosOcupacao = n;
//     } else {
//       base.precosOcupacao = {};
//       base.horaAdicionalPorPessoa = 0;
//     }
...
//     setFormDayUseModo('padrao');
//     setFormDayUsePrecoFixo(0);
//     setFormDayUsePrecoAdicional(0);
//     setFormDayUsePrecosOcupacao({});
//     setFormDayUseHoraAdicionalPorPessoa(0);
...
//       showNotification('Sem permissão.', 'error');
//       return;
//     }
//     resetCategoryForm();
...
//     const du = normalizeDayUse(cat.dayUse || {}, cat.maxPessoas);
//     setFormDayUseAtivo(du.ativo);
//     setFormDayUseModo(du.modo);
//     setFormDayUsePrecoFixo(du.precoFixo);
//     setFormDayUsePrecoAdicional(du.precoAdicional);
//     setFormDayUsePrecosOcupacao(du.precosOcupacao || {});
//     setFormDayUseHoraAdicionalPorPessoa(du.horaAdicionalPorPessoa || 0);
...
//     const dayUsePayload = normalizeDayUse({
//       ativo: formDayUseAtivo,
//       modo: formDayUseModo,
//       precoFixo: formDayUsePrecoFixo,
//       precoAdicional: formDayUsePrecoAdicional,
//       precosOcupacao: formDayUsePrecosOcupacao,
//       horaAdicionalPorPessoa: formDayUseHoraAdicionalPorPessoa,
//     }, formMaxPessoas);
...
//     if (m === 'Por quarto (tarifa fixa)') setFormPrecosOcupacao({});
//     else setFormPrecoFixo(0);
//   };
...
//       const n = {};
//       for (let i = 1; i <= max; i++) n[i] = formPrecosOcupacao[i] || 0;
//       setFormPrecosOcupacao(n);
//     }

//     if (formDayUseModo === 'ocupacao') {
//       const n2 = {};
//       for (let i = 1; i <= max; i++) n2[i] = formDayUsePrecosOcupacao[i] || 0;
//       setFormDayUsePrecosOcupacao(n2);
//     }
//   };

//   const handleDayUseModoChange = (modo) => {
//     setFormDayUseModo(modo);
//     if (modo === 'padrao') {
//       setFormDayUsePrecosOcupacao({});
//       setFormDayUseHoraAdicionalPorPessoa(0);
//     } else {
//       const n = {};
//       for (let i = 1; i <= formMaxPessoas; i++) n[i] = formDayUsePrecosOcupacao[i] || 0;
//       setFormDayUsePrecosOcupacao(n);
//     }
//   };
...
//     setSNome('');
//     setSDescricao('');
//     setSModo('data-especifica');
//     setSDataInicio('');
//     setSDataFim('');
//     setSHoraInicio('');
//     setSHoraFim('');
//     setSDiaIntegral(true);
//     setSHoraInicioCiclo('');
//     setSHoraFimCiclo('');
//     setSHoraCheckin('');
//     setSHoraCheckout('');
...
//     setSDiasMes([]);
//     setSMeses([]);
//     setSModeloCobranca('Por ocupação');
//     setSPrecoFixo(0);
//     setSPrecosOcupacao({});
//     setSMaxPessoas(5);

//     setSDayUseAtivo(false);
//     setSDayUseModo('padrao');
//     setSDayUsePreco(0);
//     setSDayUsePrecoAd(0);
//     setSDayUsePrecosOcupacao({});
//     setSDayUseHoraAdicionalPorPessoa(0);
//   };
...
//     setSModo(s.modoOperacao || 'data-especifica');

//     setSDataInicio(s.dataInicio || '');
//     setSDataFim(s.dataFim || '');
//     setSHoraInicio(s.horaInicio || '');
//     setSHoraFim(s.horaFim || '');

//     setSDiaIntegral(s.diaIntegral ?? true);
//     setSHoraInicioCiclo(s.horaInicioCiclo || '');
//     setSHoraFimCiclo(s.horaFimCiclo || '');

//     setSHoraCheckin(s.horaCheckin || '');
//     setSHoraCheckout(s.horaCheckout || '');

//     setSDiasSemana(s.diasSemana || []);
//     setSDiasMes(s.diasMes || []);
//     setSMeses(s.meses || []);

//     setSModeloCobranca(s.modeloCobranca);
//     setSPrecoFixo(s.precoFixo || 0);
//     setSPrecosOcupacao(s.precosOcupacao || {});
//     setSMaxPessoas(s.maxPessoas);

//     const du = normalizeDayUse(s.dayUse || {}, s.maxPessoas);
//     setSDayUseAtivo(du.ativo);
//     setSDayUseModo(du.modo);
//     setSDayUsePreco(du.precoFixo);
//     setSDayUsePrecoAd(du.precoAdicional);
//     setSDayUsePrecosOcupacao(du.precosOcupacao || {});
//     setSDayUseHoraAdicionalPorPessoa(du.horaAdicionalPorPessoa || 0);
...
//       ativo: sDayUseAtivo,
//       modo: sDayUseModo,
//       precoFixo: sDayUsePreco,
//       precoAdicional: sDayUsePrecoAd,
//       precosOcupacao: sDayUsePrecosOcupacao,
//       horaAdicionalPorPessoa: sDayUseHoraAdicionalPorPessoa,
//     }, sMaxPessoas);
...
//       modeloCobranca: sModeloCobranca,
//       maxPessoas: sMaxPessoas,
...
//     setSeasonalDefinitions((p) => p.filter((s) => s.id !== id));
//     showNotification('Sazonalidade removida.', 'success');
//   };

//   const handleSeasonalModeloCobrancaChange = (m) => {
//     setSModeloCobranca(m);
//     if (m === 'Por quarto (tarifa fixa)') setSPrecosOcupacao({});
//     else setSPrecoFixo(0);
//   };
...
//       const n = {};
//       for (let i = 1; i <= max; i++) n[i] = sPrecosOcupacao[i] || 0;
//       setSPrecosOcupacao(n);
//     }

//     if (sDayUseModo === 'ocupacao') {
//       const n2 = {};
//       for (let i = 1; i <= max; i++) n2[i] = sDayUsePrecosOcupacao[i] || 0;
//       setSDayUsePrecosOcupacao(n2);
//     }
//   };

//   const handleSeasonalDayUseModoChange = (modo) => {
//     setSDayUseModo(modo);
//     if (modo === 'padrao') {
//       setSDayUsePrecosOcupacao({});
//       setSDayUseHoraAdicionalPorPessoa(0);
//     } else {
//       const n = {};
//       for (let i = 1; i <= sMaxPessoas; i++) n[i] = sDayUsePrecosOcupacao[i] || 0;
//       setSDayUsePrecosOcupacao(n);
//     }
//   };
...
//               <DollarSign className="w-3.5 h-3.5" /> Gestão de tarifas por categoria, ocupação, day use e sazonalidade
//             </p>
//           </div>
...
//                 key={label}
//                 onClick={action}
//                 className={`px-4 py-2 ${theme.button} ${theme.text} rounded-lg border text-sm font-medium flex items-center gap-2 transition-colors`}
//               >
//                 {icon} {label}
//               </button>
//             ))}
...
//               <h2 className={`text-sm font-bold ${theme.text} uppercase tracking-wider`}>Visão geral</h2>
//             </div>

//             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
//               {[
//                 { label:'CATEGORIAS',          value:baseStats.totalCategorias,    sub:'Tipos ativos',           Icon:Tag      },
//                 { label:'DAY USE',             value:baseStats.categoriasDayUse,   sub:'Categorias com Day Use', Icon:Clock    },
//                 { label:'SAZONALIDADES',       value:baseStats.totalSazonalidades, sub:'Períodos cadastrados',   Icon:Calendar },
//                 { label:'COM SAZONAIS ATIVAS', value:baseStats.categoriasSazonais, sub:'Categorias configuradas',Icon:Users    },
//               ].map(({ label, value, sub, Icon }) => (
//                 <div key={label} className={`${theme.sectionBg} rounded-lg border p-4`}>
//                   <div className="flex items-center justify-between mb-2">
//                     <span className={`text-xs ${theme.textSecondary} uppercase font-medium`}>{label}</span>
//                     <Icon className="w-4 h-4 text-slate-400" />
//                   </div>
//                   <p className={`text-3xl font-bold ${theme.text}`}>{value}</p>
//                   <p className={`text-xs ${theme.textSecondary} mt-1`}>{sub}</p>
//                 </div>
//               ))}
//             </div>
//           </div>
...
//               <p className={`${theme.textSecondary} text-xs`}>Defina preços, Day Use e sazonalidades vinculadas.</p>
//             </div>
//             <div className="flex gap-2">
//               {permissions.cadastrarSazonalidade && (
//                 <button
//                   onClick={handleOpenNewSeasonal}
//                   className={`px-3 py-2 ${theme.button} ${theme.text} rounded-lg border text-sm font-medium flex items-center gap-2 transition-colors`}
//                 >
...
//               const isCollapsed = collapsedCategories[cat.id] !== false; // default: true (fechado)
//               const du = normalizeDayUse(cat.dayUse || {}, cat.maxPessoas);
...
//                       <ChevronDown className={`w-4 h-4 text-slate-500 transition-transform duration-200 mt-0.5 ${isCollapsed ? '-rotate-90' : ''}`} />
//                       <div>
//                         <div className="flex items-center gap-2 flex-wrap">
//                           <span className={`${theme.text} font-semibold text-sm`}>{cat.nome}</span>
//                           <span className={`text-[11px] px-2 py-0.5 rounded-full border ${theme.chip}`}>{cat.modeloCobranca}</span>
//                           {du.ativo && <span className={`text-[11px] px-2 py-0.5 rounded-full border ${theme.chip}`}>Day Use</span>}
//                           {(cat.sazonaisAtivas || []).length > 0 && (
//                             <span className={`text-[11px] px-2 py-0.5 rounded-full border ${theme.chip}`}>
//                               {(cat.sazonaisAtivas || []).length} sazonalidade(s)
//                             </span>
//                           )}
//                         </div>
...
//                           className={`px-3 py-1.5 text-xs rounded-lg border ${theme.button} ${theme.text} flex items-center gap-1.5 transition-colors`}
//                         >
//                           <Edit className="w-3.5 h-3.5" /> Abrir
//                         </button>
//                       )}
//                     </div>
//                   </div>
...
//                           <span className={`${theme.text} text-xs font-semibold uppercase`}>Hospedagem</span>
//                         </div>

//                         {cat.modeloCobranca === 'Por quarto (tarifa fixa)' ? (
//                           <div>
//                             <p className={`${theme.text} font-bold text-lg`}>{fmt(cat.precoFixo)}</p>
//                             <p className={`${theme.textSecondary} text-[11px] mt-1`}>Tarifa fixa por quarto.</p>
//                           </div>
//                         ) : (
//                           <div className={`rounded-lg overflow-hidden border ${theme.divider} text-xs flex-1`}>
//                             <div className={`flex ${theme.tableHeader}`}>
//                               <div className={`w-1/2 px-2 py-1 border-r ${theme.divider}`}>
//                                 <span className={`${theme.textSecondary} uppercase text-[10px]`}>Pessoas</span>
//                               </div>
//                               <div className="w-1/2 px-2 py-1">
//                                 <span className={`${theme.textSecondary} uppercase text-[10px]`}>Preço/noite</span>
//                               </div>
//                             </div>
//                             {Array.from({ length: cat.maxPessoas }).map((_, idx) => {
//                               const q = idx + 1;
//                               return (
//                                 <div key={q} className={`flex border-t ${theme.divider}`}>
//                                   <div className={`w-1/2 px-2 py-1 border-r ${theme.divider}`}>
//                                     <span className={`${theme.text} text-xs`}>{q} pax</span>
//                                   </div>
//                                   <div className="w-1/2 px-2 py-1">
//                                     <span className={`${theme.text} text-xs font-semibold`}>{fmt(cat.precosOcupacao?.[q])}</span>
//                                   </div>
//                                 </div>
//                               );
//                             })}
...
//                             <span className={`${theme.text} text-xs font-semibold uppercase`}>Day Use</span>
//                           </div>
//                           <span className={`text-[11px] px-2 py-0.5 rounded-full border ${theme.chip}`}>
...
//                               <div className="flex justify-between"><span className={theme.textSecondary}>Preço base</span><span className={`${theme.text} font-semibold`}>{fmt(du.precoFixo)}</span></div>
//                               <div className="flex justify-between"><span className={theme.textSecondary}>Hora adicional</span><span className={`${theme.text} font-semibold`}>{fmt(du.precoAdicional)}</span></div>
//                             </div>
//                           ) : (
//                             <div className="space-y-2 text-xs flex-1">
//                               <div className="flex justify-between"><span className={theme.textSecondary}>Hora adicional/pessoa</span><span className={`${theme.text} font-semibold`}>{fmt(du.horaAdicionalPorPessoa)}</span></div>
...
//                                   <div className="w-1/2 px-2 py-1"><span className={`${theme.textSecondary} uppercase text-[10px]`}>Preço</span></div>
//                                 </div>
//                                 {Array.from({ length: cat.maxPessoas }).map((_, idx) => {
//                                   const q = idx + 1;
//                                   return (
...
//                                       <div className="w-1/2 px-2 py-1"><span className={`${theme.text} text-xs font-semibold`}>{fmt(du.precosOcupacao?.[q])}</span></div>
//                                     </div>
//                                   );
//                                 })}
//                               </div>
...
//                           <div className="flex items-center gap-2 mb-2">
//                             <Calendar className="w-4 h-4 text-indigo-500" />
//                             <span className={`${theme.text} text-xs font-semibold uppercase`}>Sazonais ativas</span>
//                           </div>
//                           {(cat.sazonaisAtivas || []).length === 0 ? (
//                             <p className={`${theme.textSecondary} text-xs`}>Nenhuma sazonal ativa.</p>
//                           ) : (
...
//                                       <span className={`text-[10px] px-1.5 py-0.5 rounded-full border ${theme.chip}`}>
//                                         {modoLabel[s.modoOperacao] || s.modoOperacao}
//                                       </span>
//                                     </div>
...
//                               ? <span className={`${theme.textSecondary} text-xs`}>Nenhum vinculado.</span>
//                               : quartosVinc.map((r) => (
//                                 <span key={r.numero} className={`px-2 py-0.5 rounded-full border ${theme.chip} text-xs`}>
...
//                 <p className={`${theme.textSecondary} text-sm`}>Nenhuma sazonalidade cadastrada.</p>
//               </div>
...
//                 <div key={s.id}>
//                   <div
//                     onClick={() => toggleSeaCollapse(s.id)}
//                     className={`px-4 py-3 flex items-start justify-between cursor-pointer ${theme.cardHover} transition-colors`}
//                   >
//                     <div className="flex items-center gap-3">
//                       <ChevronDown className={`w-4 h-4 text-slate-500 transition-transform duration-200 mt-0.5 ${expanded ? '' : '-rotate-90'}`} />
//                       <div>
//                         <div className="flex items-center gap-2 flex-wrap">
//                           <span className={`${theme.text} font-semibold text-sm`}>{s.nome}</span>
//                           <span className={`text-[11px] px-2 py-0.5 rounded-full border ${theme.chip}`}>{modoLabel[s.modoOperacao] || s.modoOperacao}</span>
//                           <span className={`text-[11px] px-2 py-0.5 rounded-full border ${theme.chip}`}>{s.modeloCobranca}</span>
//                           {du.ativo && <span className={`text-[11px] px-2 py-0.5 rounded-full border ${theme.chip}`}>Day Use</span>}
//                         </div>
//                         <p className={`${theme.textSecondary} text-xs mt-0.5`}>{s.descricao}</p>
//                         <p className={`${theme.textMuted} text-[11px] mt-0.5`}>{describeSchedule(s)}</p>
//                       </div>
//                     </div>
...
//                         <>
//                           <button
//                             onClick={(e) => { e.stopPropagation(); handleEditSeasonal(s); }}
//                             className={`px-3 py-1.5 text-xs rounded-lg border ${theme.button} ${theme.text} flex items-center gap-1.5 transition-colors`}
//                           >
//                             <Edit className="w-3.5 h-3.5" /> Editar
//                           </button>
//                           <button
//                             onClick={(e) => { e.stopPropagation(); handleDeleteSeasonal(s.id); }}
//                             className="px-3 py-1.5 text-xs rounded-lg border border-rose-500/30 bg-rose-500/10 text-rose-700 dark:text-rose-300 flex items-center gap-1.5 transition-colors hover:bg-rose-500/15"
//                           >
...
//                         <div className="flex items-center gap-2 mb-2">
//                           <Clock className="w-4 h-4 text-indigo-500" />
//                           <span className={`${theme.text} text-xs font-semibold uppercase`}>Agendamento</span>
//                         </div>

//                         <div className="space-y-1.5 text-xs">
//                           <div className="flex justify-between">
//                             <span className={theme.textSecondary}>Modo</span>
//                             <span className={`${theme.text} font-medium`}>{modoLabel[s.modoOperacao]}</span>
//                           </div>
...
//                                   {(s.diasSemana || []).map((d) => (
//                                     <span key={d} className={`px-1.5 py-0.5 rounded border ${theme.chip} text-[10px]`}>
//                                       {DIAS_SEMANA[d]}
//                                     </span>
//                                   ))}
//                                 </div>
...
//                                 <div className="flex flex-wrap gap-1 justify-end">
//                                   {(s.meses || []).map((m) => (
//                                     <span key={m} className={`px-1.5 py-0.5 rounded border ${theme.chip} text-[10px]`}>
//                                       {MESES[m]}
//                                     </span>
//                                   ))}
//                                 </div>
...
//                           <span className={`${theme.text} text-xs font-semibold uppercase`}>Tabela de Preços</span>
//                         </div>

//                         {s.modeloCobranca === 'Por quarto (tarifa fixa)' ? (
//                           <div>
//                             <p className={`${theme.text} font-bold text-lg`}>{fmt(s.precoFixo)}</p>
//                             <p className={`${theme.textSecondary} text-[11px]`}>Valor fixo por quarto.</p>
//                           </div>
//                         ) : (
//                           <div className={`rounded-lg overflow-hidden border ${theme.divider} text-xs`}>
//                             <div className={`flex ${theme.tableHeader}`}>
...
//           <div className={`${isDark ? 'bg-slate-950 border-white/10' : 'bg-white border-slate-200'} rounded-xl border shadow-2xl max-w-4xl w-full my-8`}>
...
//               <button onClick={() => setShowCategoryModal(false)} className={`${theme.textSecondary} transition-colors hover:text-slate-900 dark:hover:text-white`}>
//                 <X className="w-5 h-5" />
//               </button>
//             </div>
...
//                 <div>
//                   <label className={`block mb-1 text-xs font-medium ${theme.textSecondary}`}>Nome *</label>
//                   <input type="text" value={formNome} onChange={(e) => setFormNome(e.target.value)} className={inputCls} placeholder="Ex: Standard até 5 pessoas" />
//                 </div>
//                 <div>
//                   <label className={`block mb-1 text-xs font-medium ${theme.textSecondary}`}>Máximo de pessoas</label>
//                   <input type="number" min={1} max={10} value={formMaxPessoas} onChange={(e) => handleMaxPessoasChange(parseInt(e.target.value,10))} className={inputCls} />
//                 </div>
//               </div>

//               <div>
//                 <label className={`block mb-1 text-xs font-medium ${theme.textSecondary}`}>Descrição</label>
//                 <textarea rows={2} value={formDescricao} onChange={(e) => setFormDescricao(e.target.value)} className={`${inputCls} resize-none`} />
//               </div>
...
//                           sel ? 'bg-indigo-600 text-white border-indigo-500/40' : `${theme.button} ${theme.text}`
//                         }`}
//                       >
//                         {formatRoomNumber(r.numero)}
//                       </button>
...
//                       key={value}
//                       value={value}
//                       current={formModeloCobranca}
//                       onChange={handleModeloCobrancaChange}
//                       label={label}
//                       description={desc}
//                       icon={icon}
//                       color="indigo"
...
//                 <div>
//                   <label className={`block mb-1 text-xs font-medium ${theme.textSecondary}`}>Preço fixo (R$)</label>
//                   <input type="number" step="0.01" value={formPrecoFixo} onChange={(e) => setFormPrecoFixo(parseFloat(e.target.value)||0)} className={inputCls} />
//                 </div>
//               ) : (
//                 <div>
...
//                       return (
//                         <div key={q}>
//                           <label className={`block mb-1 text-xs ${theme.textSecondary}`}>{q} pessoa{q>1?'s':''}</label>
//                           <input
//                             type="number"
//                             step="0.01"
//                             value={formPrecosOcupacao[q]||0}
//                             onChange={(e) => setFormPrecosOcupacao((p) => ({ ...p, [q]: parseFloat(e.target.value)||0 }))}
//                             className={inputCls}
//                           />
//                         </div>
//                       );
...
//                   <div className="flex items-center gap-2">
//                     <Clock className="w-4 h-4 text-indigo-500" />
//                     <span className={`${theme.text} text-sm font-semibold`}>Day Use</span>
//                   </div>

//                   <div className="flex gap-2">
//                     <RadioPill value={true}  current={formDayUseAtivo} onChange={setFormDayUseAtivo} label="Ativo"   activeClass="bg-indigo-600 text-white border-indigo-500/40" />
//                     <RadioPill value={false} current={formDayUseAtivo} onChange={setFormDayUseAtivo} label="Inativo" activeClass="bg-slate-700 text-white border-slate-500/40" />
//                   </div>
//                 </div>
...
//                   <>
//                     <div className="flex gap-2">
//                       <RadioPill value="padrao"   current={formDayUseModo} onChange={handleDayUseModoChange} label="Padrão (base + hora adicional)" activeClass="bg-indigo-600 text-white border-indigo-500/40" />
//                       <RadioPill value="ocupacao" current={formDayUseModo} onChange={handleDayUseModoChange} label="Por ocupação (pessoa + hora/pessoa)" activeClass="bg-indigo-600 text-white border-indigo-500/40" />
//                     </div>

//                     {formDayUseModo === 'padrao' ? (
//                       <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
//                         <div>
//                           <label className={`block mb-1 text-xs ${theme.textSecondary}`}>Preço base (R$)</label>
//                           <input type="number" step="0.01" value={formDayUsePrecoFixo} onChange={(e) => setFormDayUsePrecoFixo(parseFloat(e.target.value)||0)} className={inputCls} />
//                         </div>
//                         <div>
//                           <label className={`block mb-1 text-xs ${theme.textSecondary}`}>Hora adicional (R$)</label>
//                           <input type="number" step="0.01" value={formDayUsePrecoAdicional} onChange={(e) => setFormDayUsePrecoAdicional(parseFloat(e.target.value)||0)} className={inputCls} />
//                         </div>
//                       </div>
//                     ) : (
//                       <>
//                         <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
//                           <div>
//                             <label className={`block mb-1 text-xs ${theme.textSecondary}`}>Hora adicional por pessoa (R$)</label>
//                             <input type="number" step="0.01" value={formDayUseHoraAdicionalPorPessoa} onChange={(e) => setFormDayUseHoraAdicionalPorPessoa(parseFloat(e.target.value)||0)} className={inputCls} />
//                           </div>
//                         </div>

//                         <div>
//                           <label className={`block mb-2 text-xs font-medium ${theme.textSecondary}`}>Preços do Day Use por nº de pessoas (R$)</label>
//                           <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
//                             {Array.from({ length: formMaxPessoas }).map((_, idx) => {
//                               const q = idx + 1;
//                               return (
//                                 <div key={q}>
//                                   <label className={`block mb-1 text-xs ${theme.textSecondary}`}>{q} pax</label>
//                                   <input
//                                     type="number"
//                                     step="0.01"
//                                     value={formDayUsePrecosOcupacao[q]||0}
//                                     onChange={(e) => setFormDayUsePrecosOcupacao((p) => ({ ...p, [q]: parseFloat(e.target.value)||0 }))}
//                                     className={inputCls}
//                                   />
//                                 </div>
//                               );
...
//                   <Calendar className="w-4 h-4 text-indigo-500" />
//                   <span className={`${theme.text} text-sm font-semibold`}>Sazonalidades ativas</span>
//                 </div>

//                 {seasonalDefinitions.length === 0 ? (
//                   <p className={`${theme.textSecondary} text-xs`}>Nenhuma sazonalidade cadastrada.</p>
//                 ) : (
...
//                           key={s.id}
//                           type="button"
//                           onClick={() => toggleSeasonalAtiva(s.id)}
//                           className={`w-full flex items-center gap-3 p-2.5 rounded-lg border transition-colors text-left ${
//                             active ? 'border-indigo-500/40 bg-indigo-500/5' : `${theme.button} ${theme.text}`
//                           }`}
//                         >
//                           <div className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 ${active ? 'bg-indigo-600 border-indigo-500/40' : 'border-slate-300 dark:border-white/10'}`}>
//                             {active && <Check className="w-3 h-3 text-white" />}
//                           </div>
//                           <div className="flex-1 min-w-0">
//                             <div className="flex items-center gap-1.5 flex-wrap">
//                               <span className={`${theme.text} text-xs font-semibold`}>{s.nome}</span>
//                               <span className={`text-[10px] px-1.5 py-0.5 rounded-full border ${theme.chip}`}>{modoLabel[s.modoOperacao]}</span>
//                             </div>
...
//                 className={`flex-1 px-4 py-2 ${theme.button} ${theme.text} rounded-lg border text-sm font-medium transition-colors`}
//               >
//                 Cancelar
//               </button>
//               <button
...
//         <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-start justify-center z-50 p-4 overflow-y-auto">
//           <div className={`${isDark ? 'bg-slate-950 border-white/10' : 'bg-white border-slate-200'} rounded-xl border shadow-2xl max-w-3xl w-full my-8`}>
//             <div className={`p-4 border-b ${theme.divider} flex items-center justify-between`}>
//               <h3 className={`text-lg font-bold ${theme.text}`}>{editingSeasonal ? 'Editar sazonalidade' : 'Nova sazonalidade'}</h3>
//               <button onClick={() => setShowSeasonalFormModal(false)} className={`${theme.textSecondary} transition-colors hover:text-slate-900 dark:hover:text-white`}>
//                 <X className="w-5 h-5" />
//               </button>
//             </div>
...
//                 <div>
//                   <label className={`block mb-1 text-xs font-medium ${theme.textSecondary}`}>Nome *</label>
//                   <input type="text" value={sNome} onChange={(e) => setSNome(e.target.value)} className={inputCls} placeholder="Ex: Alta Temporada Verão" />
//                 </div>
//                 <div>
//                   <label className={`block mb-1 text-xs font-medium ${theme.textSecondary}`}>Máx. pessoas</label>
//                   <input type="number" min={1} max={10} value={sMaxPessoas} onChange={(e) => handleSeasonalMaxPessoasChange(parseInt(e.target.value,10))} className={inputCls} />
//                 </div>
//               </div>

//               <div>
//                 <label className={`block mb-1 text-xs font-medium ${theme.textSecondary}`}>Descrição</label>
//                 <textarea rows={2} value={sDescricao} onChange={(e) => setSDescricao(e.target.value)} className={`${inputCls} resize-none`} />
//               </div>
...
//                 <div className={`rounded-xl border ${theme.sectionBg} p-4 space-y-3`}>
//                   <p className={`text-xs ${theme.textSecondary} flex items-center gap-1.5`}>
//                     <Calendar className="w-3.5 h-3.5" /> Período com data de início e fim definidos.
//                   </p>
...
//                   <p className={`text-xs ${theme.textSecondary} flex items-center gap-1.5`}>
//                     <Clock className="w-3.5 h-3.5" /> Aplicado todos os dias. Defina se é integral ou com horário de ciclo.
//                   </p>

//                   <div>
//                     <label className={`block mb-2 text-xs font-medium ${theme.textSecondary}`}>Tipo de ciclo</label>
//                     <div className="flex gap-2">
//                       <RadioPill value={true}  current={sDiaIntegral} onChange={setSDiaIntegral} label="Integral" activeClass="bg-indigo-600 text-white border-indigo-500/40" />
//                       <RadioPill value={false} current={sDiaIntegral} onChange={setSDiaIntegral} label="Horário definido" activeClass="bg-indigo-600 text-white border-indigo-500/40" />
//                     </div>
//                   </div>
...
//                         <ToggleChip key={idx} active={sDiasSemana.includes(idx)} onClick={() => toggleDiaSemana(idx)} label={dia} />
//                       ))}
//                     </div>
//                   </div>
...
//                         <button type="button" onClick={() => setSDiasMes(DIAS_MES)} className={`text-[11px] px-2 py-0.5 rounded border ${theme.chip} transition-colors`}>Todos</button>
//                         <button type="button" onClick={() => setSDiasMes([])} className={`text-[11px] px-2 py-0.5 rounded border ${theme.button} ${theme.textSecondary} transition-colors`}>Limpar</button>
//                       </div>
//                     </div>

//                     <div className="grid grid-cols-7 gap-1.5">
//                       {DIAS_MES.map((d) => (
//                         <ToggleChip key={d} active={sDiasMes.includes(d)} onClick={() => toggleDiaMes(d)} label={String(d)} />
//                       ))}
//                     </div>
//                   </div>
...
//                       <div className="flex gap-1.5">
//                         <button type="button" onClick={() => setSMeses(MESES.map((_,i)=>i))} className={`text-[11px] px-2 py-0.5 rounded border ${theme.chip} transition-colors`}>Todos</button>
//                         <button type="button" onClick={() => setSMeses([])} className={`text-[11px] px-2 py-0.5 rounded border ${theme.button} ${theme.textSecondary} transition-colors`}>Limpar</button>
//                       </div>
//                     </div>

//                     <div className="grid grid-cols-4 gap-2">
//                       {MESES.map((mes, idx) => (
//                         <ToggleChip key={idx} active={sMeses.includes(idx)} onClick={() => toggleMes(idx)} label={mes} />
//                       ))}
//                     </div>
//                   </div>
...
//                       key={value}
//                       value={value}
//                       current={sModeloCobranca}
//                       onChange={handleSeasonalModeloCobrancaChange}
//                       label={label}
//                       description={desc}
//                       icon={icon}
//                       color="indigo"
...
//                 <div>
//                   <label className={`block mb-1 text-xs font-medium ${theme.textSecondary}`}>Preço fixo (R$)</label>
//                   <input type="number" step="0.01" value={sPrecoFixo} onChange={(e) => setSPrecoFixo(parseFloat(e.target.value)||0)} className={inputCls} />
//                 </div>
//               ) : (
//                 <div>
//                   <label className={`block mb-2 text-xs font-medium ${theme.textSecondary}`}>Preços por nº de pessoas (R$)</label>
...
//                             type="number"
//                             step="0.01"
//                             value={sPrecosOcupacao[q]||0}
//                             onChange={(e) => setSPrecosOcupacao((p) => ({ ...p, [q]: parseFloat(e.target.value)||0 }))}
//                             className={inputCls}
//                           />
//                         </div>
//                       );
...
//                   </div>
//                   <div className="flex gap-2">
//                     <RadioPill value={true}  current={sDayUseAtivo} onChange={setSDayUseAtivo} label="Ativo"   activeClass="bg-indigo-600 text-white border-indigo-500/40" />
//                     <RadioPill value={false} current={sDayUseAtivo} onChange={setSDayUseAtivo} label="Inativo" activeClass="bg-slate-700 text-white border-slate-500/40" />
//                   </div>
//                 </div>
...
//                   <>
//                     <div className="flex gap-2">
//                       <RadioPill value="padrao"   current={sDayUseModo} onChange={handleSeasonalDayUseModoChange} label="Padrão (base + hora adicional)" activeClass="bg-indigo-600 text-white border-indigo-500/40" />
//                       <RadioPill value="ocupacao" current={sDayUseModo} onChange={handleSeasonalDayUseModoChange} label="Por ocupação (pessoa + hora/pessoa)" activeClass="bg-indigo-600 text-white border-indigo-500/40" />
//                     </div>
...
//                           <div>
//                             <label className={`block mb-1 text-xs ${theme.textSecondary}`}>Hora adicional por pessoa (R$)</label>
//                             <input type="number" step="0.01" value={sDayUseHoraAdicionalPorPessoa} onChange={(e) => setSDayUseHoraAdicionalPorPessoa(parseFloat(e.target.value)||0)} className={inputCls} />
//                           </div>
//                         </div>
...
//                                     type="number"
//                                     step="0.01"
//                                     value={sDayUsePrecosOcupacao[q]||0}
//                                     onChange={(e) => setSDayUsePrecosOcupacao((p) => ({ ...p, [q]: parseFloat(e.target.value)||0 }))}
//                                     className={inputCls}
//                                   />
//                                 </div>
//                               );
...
//             <div className={`p-4 border-t ${theme.divider} flex gap-2`}>
//               <button onClick={() => setShowSeasonalFormModal(false)} className={`flex-1 px-4 py-2 ${theme.button} ${theme.text} rounded-lg border text-sm font-medium transition-colors`}>Cancelar</button>
//               <button onClick={handleSaveSeasonal} className="flex-1 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors">Salvar</button>
//             </div>
//           </div>
//         </div>
//       )}
...
//           <div className={`${isDark ? 'bg-slate-950 border-white/10' : 'bg-white border-slate-200'} rounded-xl border shadow-2xl max-w-md w-full`}>
//             <div className={`p-4 border-b ${theme.divider} flex items-center justify-between`}>
//               <div className="flex items-center gap-2">
//                 <Shield className="w-5 h-5 text-indigo-500" />
//                 <h3 className={`text-lg font-bold ${theme.text}`}>Permissões de Preços</h3>
//               </div>
//               <button onClick={() => setShowPermissionsModal(false)} className={`${theme.textSecondary} transition-colors hover:text-slate-900 dark:hover:text-white`}>
//                 <X className="w-5 h-5" />
//               </button>
//             </div>
...
//               <button onClick={() => setShowPermissionsModal(false)} className="w-full px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors">
//                 Confirmar
//               </button>
//             </div>
//           </div>
...
//           <div className={`${notification.type==='success'?'bg-emerald-600':notification.type==='error'?'bg-rose-600':notification.type==='warning'?'bg-amber-600':'bg-slate-800'} text-white px-6 py-3 rounded-lg flex items-center gap-3 shadow-lg`}>
...
//         select option { background: ${isDark ? '#020617' : '#ffffff'}; color: ${isDark ? 'white' : '#0f172a'}; }
